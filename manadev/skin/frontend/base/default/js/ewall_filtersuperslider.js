 ...
 */
;var Ewall = Ewall || {};
var _ewall_oldResizehandler = {};
var _ewall_sliderTimers = {};
Ewall.filterSuperSlider = function(id, o) {
    var _changing = false;
    ...
    function _resizeSpanAndHandles() {
        ...
        if (!_ewall_sliderTimers[id]) {
            _ewall_sliderTimers[id] = setInterval(function () {
                if (s.needsResize()) {
                    ...
                    //console.log('stable');
                    clearInterval(_ewall_sliderTimers[id]);
                    _ewall_sliderTimers[id] = null;
                }
                else {
    ...
    if (_ewall_oldResizehandler[id]) {
        jQuery(window).unbind('resize', _ewall_oldResizehandler[id]);
        _ewall_oldResizehandler[id] = null;
    }
    _ewall_oldResizehandler[id] = _resizeSpanAndHandles;
    jQuery(_resizeSpanAndHandles);

    jQuery(window).bind('resize', _resizeSpanAndHandles);
    //jQuery('body').click(_resizeSpanAndHandles);
};
Ewall.filterAttributeSlider = function (id, o) {
...
Ewall.filterRangeInput = function (id, o) {
    var _changing = false;
    var $_from = jQuery('#' + id + '-applied input.m-slider.m-from');
    var $_to = jQuery('#' + id + '-applied input.m-slider.m-to');
    var _value = [
